...
export class AdminAuthService {
  ...
  private readonly adminLoginUrl = '/users/admin/login';

  constructor(private http: HttpClient) {}

  /**
   * Admin Login: Sends login credentials and stores user + token if role is ADMIN
   */
  adminLogin(credentials: UserLogin): Observable<AdminLoginResponse> {
    return this.http.post<AdminLoginResponse>(this.adminLoginUrl, credentials).pipe(
      tap({
